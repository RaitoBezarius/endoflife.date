...
module EndOfLifeHooks
  ...
  BLACKLISTED_URLS = [
    'https://www.adobe.com', # always returns a Net::ReadTimeout
    'https://business.adobe.com', # always returns a Net::ReadTimeout
    'https://www.nokia.com', # all those URL trigger a timeout
    'https://www.mysql.com', # always returns a 403 Forbidden
    'https://dev.mysql.com', # always returns a 403 Forbidden
    'https://www.clamav.net', # always returns a 403 Forbidden
    'https://docs.clamav.net', # always returns a 403 Forbidden
    'https://support.fairphone.com', # always returns a 403 Forbidden
    'https://wordpress.org', # always returns an 'end of file reached' error
    'https://make.wordpress.org', # always returns an 'end of file reached' error,
    'https://github.com/nodejs/node/blob/main/doc/changelogs/CHANGELOG_V8.md#8.17.0', # often returns a 502 Bad Gateway
    'https://www.java.com/releases/', # always returns a Net::ReadTimeout
    'https://mirrors.slackware.com', # always returns a redirection forbidden
    'https://docs-prv.pcisecuritystandards.org', # always returns a 403 Forbidden
  ]
  VALID_CATEGORIES = [
    'app',
    'db',
  ...
  class Validator
    ...
    def contains_invalid_urls(markdown)
      ...
      urls.each do |url|
        begin
          ...
          declare_error('content', url, "got an error : '#{e}'")
        end
      end
    end
    ...
    def check_url(url)
      if is_blacklisted(url)
        Jekyll.logger.warn TOPIC, "Skipping blacklisted URL #{url}."
        return
      end

      user_agent = 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0'
      Jekyll.logger.debug TOPIC, "Checking URL #{url}."
      URI.open(url, 'User-Agent' => user_agent, :open_timeout => 20, :read_timeout => 20) do |response|
    ...
    def is_blacklisted(url)
      EndOfLifeHooks::BLACKLISTED_URLS.each do |blacklisted_url|
        return true if url.start_with?(blacklisted_url)
      end
